...
function GoldStandardMission({draft,updateDraft,done,markTask}:{draft:MissionDraft;updateDraft:(p:Partial<MissionDraft>)=>void;done:Record<string,boolean>;markTask:(i:number,v?:boolean)=>void}){
  ...
  const canValidate=chosen && marketScore>=9;
  return <section className="missionFramework">
    <div className="quickWinBar"><div><small>DEIN QUICK WIN</small><h3>Nach dieser Mission kannst du in einem Satz erklären, wem du hilfst, welches Problem du löst und warum ein Kunde dafür bezahlen sollte.</h3></div><span>45–60 MIN</span></div>

    <article className="premiumInsight"><span>01</span><div><small>VERSTEHEN</small><h2>Was eine profitable Nische wirklich ist</h2><p>Eine Nische ist nicht einfach eine Branche. Für eine Lead Generation Agency brauchst du einen Markt, in dem <b>ein neuer Kunde wirtschaftlich wertvoll</b> ist, Unternehmen regelmässig neue Aufträge brauchen und du Entscheider zuverlässig erreichen kannst.</p><div className="conceptGrid"><div><b>Hoher Kundenwert</b><p>Ein gewonnener Kunde sollte genug Wert haben, damit dein Service finanziell Sinn ergibt.</p></div><div><b>Messbarer Bedarf</b><p>Der Markt muss kontinuierlich neue Anfragen, Termine oder Verkaufschancen benötigen.</p></div><div><b>Erreichbare Entscheider</b><p>Du brauchst einen realistischen Weg zu Inhabern, Geschäftsführern oder Vertriebsverantwortlichen.</p></div></div><aside><strong>Was erfolgreiche Agenturen anders machen</strong><p>Sie starten eng. Eine klare Zielgruppe macht Angebot, Ansprache, Proof und Akquise deutlich einfacher. Breiter werden kannst du später.</p></aside></div></article>

    <article className="missionStep"><header><span>02</span><div><small>ENTSCHEIDEN</small><h2>Wähle deine erste Kernnische</h2></div></header><p>Wähle für den Start <b>eine</b> Nische. Du bindest dich nicht für immer – du schaffst nur einen klaren Ausgangspunkt für die nächsten Missionen.</p><div className="nicheGrid">{nicheOptions.map(x=><button key={x} className={draft.niche===x?"selected":""} onClick={()=>{updateDraft({niche:x,customNiche:""});markTask(0,true)}}>{x}<span>{draft.niche===x?"✓":"→"}</span></button>)}</div><label className="missionField"><span>Oder eigene Nische</span><input value={draft.customNiche} onChange={e=>{updateDraft({customNiche:e.target.value,niche:""});if(e.target.value.trim().length>2)markTask(0,true)}} placeholder="z. B. Schweizer Architekturbüros"/></label><div className="microResult"><small>DEINE AKTUELLE WAHL</small><b>{chosen || "Noch keine Nische gewählt"}</b></div></article>

    <article className="missionStep"><header><span>03</span><div><small>PRÜFEN</small><h2>Der 15-Punkte Money Check</h2></div></header><p>Bewerte deine Nische nicht nach Bauchgefühl. Gib jedem Faktor 1–5 Punkte. Ab <b>9/15</b> ist sie stark genug, um in der nächsten Mission weiterzuarbeiten.</p><div className="scoreQuestions"><ScoreQuestion label="Bedarf" hint="Brauchen diese Unternehmen regelmässig neue Kunden oder Termine?" value={draft.demand} onChange={v=>updateDraft({demand:v})}/><ScoreQuestion label="Budget" hint="Ist ein neuer Kunde genug wert, damit Marketing/Lead Gen bezahlt werden kann?" value={draft.budget} onChange={v=>updateDraft({budget:v})}/><ScoreQuestion label="Erreichbarkeit" hint="Kannst du Entscheider über E-Mail, LinkedIn, Telefon oder lokale Recherche finden?" value={draft.reachability} onChange={v=>updateDraft({reachability:v})}/></div><div className={`marketVerdict ${marketScore>=9?"good":marketScore?"warn":""}`}><div><small>NICHE SCORE</small><strong>{marketScore}<span>/15</span></strong></div><p>{marketScore>=12?"Sehr starke Ausgangslage. Jetzt mit realen Unternehmen validieren.":marketScore>=9?"Solide Nische. Die nächste Prüfung entscheidet.":marketScore?"Noch schwach. Prüfe eine andere Nische oder hinterfrage deine Bewertung.":"Bewerte zuerst alle drei Faktoren."}</p></div></article>

    <article className="missionStep"><header><span>04</span><div><small>MARKT VALIDIEREN</small><h2>Prüfe 10 echte Unternehmen</h2></div></header><p>Öffne Google Maps, LinkedIn oder Branchenverzeichnisse. Suche zehn Unternehmen aus deiner Nische und notiere kurz, ob du einen klaren Weg zum Entscheider und einen sichtbaren Bedarf erkennst.</p><div className="toolStrip"><span>Google Maps</span><span>LinkedIn</span><span>Firmenwebsite</span><span>Branchenverzeichnis</span></div><label className="missionField"><span>Deine 10 Unternehmen / kurze Notizen</span><textarea value={draft.companies} onChange={e=>updateDraft({companies:e.target.value})} placeholder={"1. Muster Physio AG – 2 Standorte, Kontakt Inhaber gefunden\n2. ..."}/></label><button className="stepConfirm" disabled={!canValidate || draft.companies.trim().length<30} onClick={()=>markTask(1,true)}>Marktanalyse als erledigt markieren ✓</button><div className="warningBox"><strong>⚠ Häufiger Fehler</strong><p>Nicht monatelang recherchieren. Ziel ist keine perfekte Marktstudie, sondern genug Evidenz, um mit echten Gesprächen weiterzulernen.</p></div></article>

    <article className="missionStep"><header><span>05</span><div><small>PROBLEM FINDEN</small><h2>Formuliere 5 Probleme, für die Lead Generation einen Wert hat</h2></div></header><p>Denke aus Sicht des Kunden. Nicht „zu wenig Leads“, sondern konkrete geschäftliche Folgen: leere Kalender, schwankende Auslastung, zu starke Empfehlungsabhängigkeit oder kein planbarer Vertrieb.</p><div className="exampleStrip"><span>„Zu viele freie Termine im Kalender“</span><span>„Abhängigkeit von Empfehlungen“</span><span>„Vertrieb hat keine konstante Pipeline“</span></div><label className="missionField"><span>Deine fünf Probleme</span><textarea value={draft.problems} onChange={e=>updateDraft({problems:e.target.value})} placeholder={"1. ...\n2. ...\n3. ...\n4. ...\n5. ..."}/></label><button className="stepConfirm" disabled={draft.problems.trim().length<35} onClick={()=>markTask(2,true)}>Probleme gespeichert ✓</button></article>

    <article className="missionStep positioningBuilder"><header><span>06</span><div><small>POSITIONIEREN</small><h2>Baue deine Positionierung</h2></div></header><p>Eine gute Positionierung sagt nicht, was dein Tool macht. Sie sagt <b>für wen</b> du <b>welches wertvolle Ergebnis</b> erzeugst.</p><div className="builderGrid"><label><span>Ich helfe …</span><input value={draft.targetGroup} onChange={e=>updateDraft({targetGroup:e.target.value})} placeholder={chosen || "z. B. Physiotherapiepraxen"}/></label><label><span>die aktuell …</span><input value={draft.coreProblem} onChange={e=>updateDraft({coreProblem:e.target.value})} placeholder="z. B. zu viele freie Termine haben"/></label><label><span>dabei …</span><input value={draft.outcome} onChange={e=>updateDraft({outcome:e.target.value})} placeholder="z. B. planbar qualifizierte Erstgespräche zu gewinnen"/></label><label><span>anders als …</span><input value={draft.differentiator} onChange={e=>updateDraft({differentiator:e.target.value})} placeholder="optional: dein Differenzierungsmerkmal"/></label></div><div className="positioningPreview"><small>DEIN POSITIONIERUNGS-ENTWURF</small><p>„Ich helfe <b>{draft.targetGroup || "[Zielgruppe]"}</b>, die <b>{draft.coreProblem || "[Problem]"}</b>, dabei <b>{draft.outcome || "[Ergebnis]"}</b>{draft.differentiator?`, ${draft.differentiator}`:""}.“</p></div><button className="stepConfirm gold" disabled={!positioning} onClick={()=>markTask(3,true)}>Positionierung finalisieren →</button></article>

    <article className="originAiWorkflow"><div><small>ORIGIN AI · SPÄTER DIREKT HIER</small><h2>Dein Co-Founder kennt diese Daten bereits.</h2><p>Wenn Origin AI integriert ist, musst du deine Nische nicht nochmals erklären. Die KI erhält die Informationen aus dieser Mission und kann daraus unmittelbar weiterarbeiten.</p></div><div><button disabled>◎ Nische analysieren</button><button disabled>✦ Positionierung schärfen</button><button disabled>↗ Value Proposition erstellen</button></div></article>

    <article className="missionReality"><small>REALITY CHECK</small><h3>Diese Mission macht dich nicht automatisch erfolgreich.</h3><p>Sie sorgt dafür, dass du nicht mit einer beliebigen Zielgruppe und einem austauschbaren Angebot startest. Umsatz entsteht erst, wenn du später echte Unternehmen ansprichst, lernst und konsequent umsetzt.</p></article>
  </section>
}
